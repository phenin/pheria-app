            ...
            <Flex direction={'row'}>
              {
                state.listTemplate.map((item, index) => {
                  return (
                    <TouchableOpacity onPress={()=>selectTemplate(item)} key={index}>
                      <Image width={item.width*vh} height={item.height*vh} source={{uri: REACT_APP_API + item.image}} alt={item.name}></Image>
                    </TouchableOpacity>
                  )
                })
              }
