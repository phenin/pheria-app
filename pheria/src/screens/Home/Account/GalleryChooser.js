...
const GalleryChooser = ({ navigation, route }) => {
    ...
    const [writeAltText, setWriteAltText] = useState(false)
    ...
    const _postToolScrollRef = useRef(null)
    const ref = useRef({
        enableGesture: true,
        fullSize: false,
    ...
    const _onDone = async () => {
        ...
        if (isChooseProfilePhoto) {
            ...
            await dispatch(UploadAvatarRequest(uri, extension ))
            setUploading(false)
            navigate('Account')
        } else {
            if (step === 1) {
    ...
    const _onTagChange = React.useCallback((images) => {
            ...
            <View style={{
                ...
                <TouchableOpacity
                    ...
                    <Text style={{
                        ...
                        fontWeight: '600',
                        color: '#318bfb'
                    }}>{isChooseProfilePhoto ? 'Xong' : (
                        step === 1 ? 'Tiếp theo' : 'Chia sẻ'
                    )}</Text>
                </TouchableOpacity>
            </View>
            {step === 1 && <>
