...
const ProfileX = ({ route }) => {
    const userXname = route.params.name
    ...
    const [followType, setFollowType] = useState(2)
    const ref = useRef({
        showHeaderTab: false,
        headerHeight: 0,
        currentGalleryTab: 1,
    ...
    useEffect(() => {
        const type = (userX.requestedList && userX.requestedList.indexOf(me?.name || '') > -1)
            ? 3 : (
