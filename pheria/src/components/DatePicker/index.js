...
const DatePicker = (props) => {
    ...
    const _onScrollEnd = ({ nativeEvent }, type) => {
        /**
         * TYPE:1 => SCROLLDATE
         * TYPE:2=>SCROLLMONTH
         * TYPE:3=>SCROLLYEAR
...
function scrollEventHandlers(onYearChange, onMonthIndexChange, onDateChange, activeDateIndex, setActiveDateIndex, activeMonthIndex, setActiveMonthIndex, activeYearIndex, setActiveYearIndex) {
    const _onScrollDate = ({ nativeEvent }) => {
    ...
    const _onScrollMonth = ({ nativeEvent }) => {
        const offsetY = nativeEvent.contentOffset.y;
    ...
    const _onScrollYear = ({ nativeEvent }) => {
        const offsetY = nativeEvent.contentOffset.y;
        const offsetForScroll = offsetY / 44 - Math.floor(offsetY / 44);
        let nextIndex = 0;
        if (offsetForScroll > 0.5) {
