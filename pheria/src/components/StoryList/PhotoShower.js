...
const PhotoShower = ({ sources, onChangePage }) => {
    ...
    const _onEndDragHandler = ({ nativeEvent: {contentOffset: { x }} }) => {
    ...
    const _onViewTagProfile = (username) => {
...
function getLabelOpacity(_animTags, index, index2) {
...
function getLabelHeight(_animTags, index, index2, tag) {
    return _animTags[index][index2].interpolate({
        inputRange: [0, 1],
...
function getLabelWidth(_animTags, index, index2, tag) {
    return _animTags[index][index2].interpolate({
        inputRange: [0, 1],
